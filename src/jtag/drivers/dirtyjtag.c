/***************************************************************************
 *   Copyright (C) 2026 Jean THOMAS                                        *
 *   pub0@git.jeanthomas.me                                                *
 *   
 *   Modified in 2026 by Zach Weatherley
 *   exposuremg@protonmail.com
 *
...
{
	assert(length < (size_t)dirtyjtag_buffer_size);
	assert(command != NULL);

	if (dirtyjtag_buffer_use + (int)length + 1 > dirtyjtag_buffer_size) {
...
static void syncbb_scan(bool ir_scan, enum scan_type type, uint8_t *buffer, int scan_size)
{
	...
	LOG_DEBUG_IO("syncbb_scan type=%d scan_size=%d", type, scan_size);
	LOG_DEBUG_IO("TX:");
	for (i = 0; i < xfer_bytes; i++) {
		LOG_DEBUG_IO("%02x", buffer[i]);
	}
